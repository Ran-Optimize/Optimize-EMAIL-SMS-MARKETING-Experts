...
const ProfitCalculator: React.FC = () => {
  ...
  return (
    <section className="py-24 bg-neutral-900/50 relative overflow-hidden text-right" dir="rtl">
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <div className="inline-block px-4 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm font-bold mb-4 uppercase tracking-tighter">
            ROI Analytics Tool
          </div>
          <h2 className="text-4xl md:text-6xl font-black mb-6 tracking-tight font-assistant">כמה כסף נשאר לך על השולחן?</h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto font-assistant">
            סמנו את הערוצים שחסרים לכם כרגע בחנות. אנחנו נראה לכם את פוטנציאל הצמיחה הריאלי שלכם ב-60 הימים הקרובים.
          </p>
        </div>

        <div className="grid lg:grid-cols-2 gap-8 items-stretch">
          <div className="bg-black/40 border border-white/10 p-8 md:p-12 rounded-[2.5rem] backdrop-blur-xl flex flex-col justify-between">
            <div className="mb-12">
              <label className="block text-xl font-black mb-6 font-assistant">מחזור חודשי נוכחי: ₪{turnover.toLocaleString()}</label>
              <input 
                type="range" 
                min="60000" 
                max="1000000" 
              ...
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  <button 
                    ...
                    className={`flex items-center justify-between p-6 rounded-2xl border transition-all duration-300 ${channelsToAdd[item.id as keyof typeof channelsToAdd] ? 'bg-blue-600/20 border-blue-500 ring-2 ring-blue-500/20' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}
                  >
                    <span className="flex items-center gap-4 text-lg font-bold">
                      <span className="text-2xl">{item.icon}</span>
                      {item.label}
                    </span>
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${channelsToAdd[item.id as keyof typeof channelsToAdd] ? 'bg-blue-500' : 'bg-white/10'}`}>
          ...
          <div className="relative group">
            ...
            <div className="relative h-full bg-gradient-to-br from-neutral-900 to-black border border-white/10 p-10 md:p-14 rounded-[2.5rem] shadow-2xl flex flex-col font-assistant">
              
              <div className="flex-grow space-y-12">
                <div>
                  <p className="text-blue-400 text-sm font-black uppercase tracking-[0.2em] mb-4">תוספת מחזור מוערכת ({calculation.multiplierPercent}%)</p>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-white/5 p-6 rounded-3xl border border-white/10">
                    ...
                    <p className="text-3xl font-black text-white">₪{calculation.yearlyAdded.toLocaleString()}</p>
                  </div>
                  <div className="bg-white/5 p-6 rounded-3xl border border-white/10">
                    <p className="text-gray-500 text-xs font-bold uppercase mb-2">ROAS צפוי</p>
                    <p className="text-3xl font-black text-green-400">{displayRoas}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-12 pt-8 border-t border-white/10">
                <p className="text-gray-400 mb-8 leading-relaxed font-medium">
                  אנחנו מוסיפים בממוצע <span className="text-white font-bold">27% מהמחזור הקיים</span> ללקוחות שלנו (לפחות 20%), ומבקשים <span className="text-blue-400 font-bold">אחוזים בודדים רק מהתוצאות שהבאנו</span>.
                </p>
                <a href="#contact" className={`w-full py-6 rounded-2xl font-black text-xl text-center block transition-all shadow-xl ${calculation.selectedCount > 0 ? 'bg-blue-600 text-white hover:bg-blue-500 hover:scale-[1.02]' : 'bg-neutral-800 text-neutral-500 cursor-not-allowed'}`}>
